...
async function refreshRAM() {
  ...
  try {
    ...
    if (j.ram && j.ram.percent !== undefined) {
      ...
      const total = j.ram.totalFormatted || j.ram.total;
      const used = j.ram.usedFormatted || j.ram.used;
      const free = j.ram.freeFormatted || j.ram.available;
...
async function refreshDiskSingle(mountPoint) {
  try {
    ...
    if (j.percent !== undefined) {
      // Use formatted values from backend
      const total = j.totalFormatted || j.total;
      const used = j.usedFormatted || j.used;
      const free = j.freeFormatted || j.free;
